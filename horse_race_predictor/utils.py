"""
Shared utilities for Horse Race Predictor.

Consolidates common patterns mirrored from stock_target_tracker/utils.py:
...
- load_env() / get_env(): load API keys from .env file
- rate_limit(): enforce minimum interval between requests to a source
"""
...
# Database file path (same directory as this module). Override with the
# HRP_DB_PATH env var to run the pipeline against a separate DB (e.g. tests
# pointing at a temp DB without touching the main horse_tracker.db).
DB_PATH = os.environ.get(
    "HRP_DB_PATH",
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "horse_tracker.db"),
)
...
def run_and_save(main_func, prefix='horse_tracker_', output_dir=None):
...
def load_env():
    ...
    Looks for .env in the horse_race_predictor/ directory first, then the
...
def rate_limit(source_name, min_interval=1.0):
    ...
    Args:
        source_name: Name of the source (e.g. 'equibase', 'drf_free').
