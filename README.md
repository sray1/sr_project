...
- [`horse_race_predictor/`](horse_race_predictor/README.md) - Horse racing consensus predictor (manual-input expert-pick aggregation for a today's race) + automated backtest pipeline that scores naive baselines (MLO favorite, post position, leading jockey/trainer, random, consensus) against HRN results across a date window and emits an HTML accuracy/ROI report (see [full README](horse_race_predictor/README.md))